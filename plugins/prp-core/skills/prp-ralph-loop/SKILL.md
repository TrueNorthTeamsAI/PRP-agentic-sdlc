...
- The shim is idempotent: running it twice on the same tree always reaches the same state. Safe to run on every Ralph iteration — once V4 is reached, every subsequent invocation is a silent no-op.
